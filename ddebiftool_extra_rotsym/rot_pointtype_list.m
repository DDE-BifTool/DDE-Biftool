...
fcnarr=struct(...
    'stst',  {0, ev,      estab, @(p)0},...
    'psol',  {0, floquet, pstab, @(p)[1;1]},...
    'RWFold',{1, ev,      estab, @(p)[0;0]},...
    'hopf',  {1, ev,      estab, @(p)[0;1i*get_omega(p)*[-1;1]]},...
    'POfold',{1, floquet, pstab, @(p)[1;1;1]},...
    'torus', {1, floquet, pstab, @(p)[1;1;exp(1i*p.omega*pi*[-1;1])]},...
    'PD',    {1, floquet, pstab, @(p)[1;1;-1]},...
    'hcli',  {1, @(p)0,   pstab, @(p)[1;1]},...
    'hoho',  {2, ev,      estab, @(p)1i*[0;omega_i(p_tohoho(p),1)*[-1;1];omega_i(p_tohoho(p),2)*[-1;1]]},...
    'zeho',  {2, ev,      estab, @(p)[0;0;1i*p.nvec.omega*[-1;1]]},...
    'genh',  {2, ev,      estab, @(p)1i*p.omega*[0;-1;1]},...
    'CP',    {2, ev,      estab, @(p)[0;0]},...
    'cusp',  {2, ev,      estab, @(p)[0;0]},...
    'BT',    {2, ev,      estab, @(p)[0;0;0]});
