...
if length(nx) == 1 && isempty(np) && isempty(v)
	switch nx
	case 0
		J = [-ones(size(xx(1,1,:))), zeros(size(xx(1,1,:))); zeros(size(xx(1,1,:))), -ones(size(xx(1,1,:)))];
	case 1
		J = [-par(1)*par(2)*(1-tanh(par(2)*xx(1,2,:)-1).^2)*cosh(1)^2, 0*xx(1,2,:); 0*xx(1,2,:), -par(1)*par(2)*(1-tanh(par(2)*xx(2,2,:)-1).^2)*cosh(1)^2];
	case 2
		J = [0*xx(1,2,:), par(3)*par(4)*(1-tanh(par(4)*xx(2,3,:)-1).^2)*cosh(1)^2; par(3)*par(4)*(1-tanh(par(4)*xx(1,3,:)-1).^2)*cosh(1)^2, 0*xx(1,2,:)];
	end
...
elseif length(nx) == 1 && length(np) == 1 && isempty(v)
	...
	case 1
		switch np
		case 1
			J = [-par(2)*(1-tanh(par(2)*xx(1,2,:)-1).^2)*cosh(1)^2, zeros(size(xx(1,1,:))); zeros(size(xx(1,1,:))), -par(2)*(1-tanh(par(2)*xx(2,2,:)-1).^2)*cosh(1)^2];
		case 2
	...
	case 2
		...
		case 2
			J = [0, 0; 0, 0];
		case 3
			J = [zeros(size(xx(1,1,:))), par(4)*(1-tanh(par(4)*xx(2,3,:)-1).^2)*cosh(1)^2; par(4)*(1-tanh(par(4)*xx(1,3,:)-1).^2)*cosh(1)^2, zeros(size(xx(1,1,:)))];
		case 4
...
elseif length(nx) == 2 && isempty(np) && ~isempty(v)
	...
	case 1
		...
		case 0
			J = [zeros(size(xx(1,1,:))), zeros(size(xx(1,1,:))); zeros(size(xx(1,1,:))), zeros(size(xx(1,1,:)))];
		case 1
			J = [2*par(1)*par(2)^2*tanh(par(2)*xx(1,2,:)-1).*(1-tanh(par(2)*xx(1,2,:)-1).^2)*cosh(1)^2.*v(1,:,:), zeros(size(xx(1,1,:))); zeros(size(xx(1,1,:))), 2*par(1)*par(2)^2*tanh(par(2)*xx(2,2,:)-1).*(1-tanh(par(2)*xx(2,2,:)-1).^2)*cosh(1)^2.*v(2,:,:)];
		case 2
			J = [zeros(size(xx(1,1,:))), zeros(size(xx(1,1,:))); zeros(size(xx(1,1,:))), zeros(size(xx(1,1,:)))];
		end
	case 2
		...
		case 1
			J = [zeros(size(xx(1,1,:))), zeros(size(xx(1,1,:))); zeros(size(xx(1,1,:))), zeros(size(xx(1,1,:)))];
		case 2
			J = [zeros(size(xx(1,1,:))), -2*par(3)*par(4)^2*tanh(par(4)*xx(2,3,:)-1).*(1-tanh(par(4)*xx(2,3,:)-1).^2)*cosh(1)^2.*v(2,:,:); -2*par(3)*par(4)^2*tanh(par(4)*xx(1,3,:)-1).*(1-tanh(par(4)*xx(1,3,:)-1).^2)*cosh(1)^2.*v(1,:,:), zeros(size(xx(1,1,:)))];
		end
	end
end
if isempty(J)
