function s = gen_systemfiles_octave(f,n,p,free_pars,taus,sys_dir,...
...
% @Id $Id: gen_systemfiles_octave.m 309 2018-10-28 19:02:42Z jansieber $
%
%% set defaults
m = length(taus)+1;
par = sym('par', [1,p+m-1]);
...
for i=1:n
    str=strcat(str,sprintf('f(%d,:) = ',i),matlabchar(f(i)),';\n');
end
str=strcat(str,'\nend\n');
...
for nx=0:m-1
    ...
    for i=1:n
        for j=1:n
            ...
            str = strcat(str,matlabchar(deri));
            if j < n
                str = strcat(str,', ');
            end
        end
...
if vectorized
    ...
    for nx=0:m-1
        ...
        for i=1:n
            for j=1:n
                deri = diff(f(i), xx(j,nx+1));
                str = strcat(str,'(',matlabchar(deri),'.*I)');
                if j < n
                    str = strcat(str,', ');
                end
            end
...
if generate_multiplinearforms
    ...
    % wrap matlabFunction into try catch, otherwise an error will
    % be generated after the file as been written
    try    
...
function s = matlabchar(f)
    s=func2str(function_handle(f));
    expression = '@\(.+?\)';
    replace = '';
