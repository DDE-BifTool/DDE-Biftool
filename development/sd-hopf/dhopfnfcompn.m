...
% only works for constant delay DDE so we implelement our expanded
...
%% which Hopf-Hopf point to find
HHj=3;
Hbrch=2*HHj-1; %HHj for j=1,2,3 found on branches 1,3 and 5 
...
disp(['Computing Hopf curve ',num2str(Hbrch)])
start_ind=fpi(bif2num('hopf'),Hbrch);  %% compute H_Hbrch curve

[hopfbr1, suc] = SetupHopf(funcs,eqbrbifs, start_ind-1, 'contpar', [3 4], 'dir',4); %, 'step', cstepsize);
...
disp(['Detecting bifurcations from Hopf curve ',num2str(Hbrch)])
disp(['Looking for HH',num2str(HHj)])
...
for i=1:numel(WherePerBifs)
    ...
    if abs(eqnunst(WherePerBifs(i))-eqnunst(WherePerBifs(i)+1))==2
        disp('Refining potential Hopf-Hopf point')       
        ttoc=toc;
