...
nfuncs=set_funcs(...
    'sys_rhs',@(x,p)poscontrol_rhs(x,p,ind),...
    'sys_tau',@(it,x,p)poscontrol_tau(it,x,p,ind),...
    'sys_ntau',@()3,...
    'x_vectorized',true,'p_vectorized',true);
sfuncs=set_symfuncs('sym_poscontrol');
%% initial parameters
tau0=0.1;
s0=1;
c=2;
...
contpar=ind.tau0;
%%
eqs=SetupStst(nfuncs,'x',x0,'parameter',par0,'step',0.1,...
    'contpar',contpar,'max_step',[contpar,0.05],'max_bound',[contpar,4]);
figure(1);clf;ax1=gca;
eqs=br_contn(nfuncs,eqs,100,'plotaxis',ax1);
%%
[eqss,testsfuncs,sbifind,sbiftype]=LocateSpecialPoints(sfuncs,eqs);
[eqsn,testnfuncs,nbifind,nbiftype]=LocateSpecialPoints(nfuncs,eqs);
%%
hopf=SetupHopf(sfuncs,eqss,sbifind(1),'contpar',[ind.tau0,ind.s0],'dir',ind.s0,...
    'min_bound',[ind.tau0,0;ind.s0,0],'step',1e-3,'max_step',[0,1e-1]);
figure(2);clf;ax2=gca;
hopf=br_contn(sfuncs,hopf,100,'plotaxis',ax2);
hopf=br_rvers(hopf);
hopf=br_contn(sfuncs,hopf,100,'plotaxis',ax2);
%%
[shopf_wbifs,shopffuncs,sbif2ind,sbif2type]=LocateSpecialPoints(sfuncs,hopf);
[nhopf_wbifs,nhopffuncs,nbif2ind,nbif2type]=LocateSpecialPoints(nfuncs,hopf);
%%
sgenh=nmfm_genh(sfuncs,p_togenh(shopf_wbifs.point(sbif2ind)),'free_pars',[ind.tau0,ind.s0]);
ngenh=nmfm_genh(nfuncs,p_togenh(shopf_wbifs.point(sbif2ind)),'free_pars',[ind.tau0,ind.s0]);
%%
[LPCfuncs,LPCbranch] = C1branch_from_C2point(sfuncs,sgenh,[ind.tau0,ind.s0],...
    'codim2','genh','codim1','POfold','print_residual_info',1);
%%
[LPCfuncs,LPCbranch] = C1branch_from_C2point(nfuncs,ngenh,[ind.tau0,ind.s0],...
    'codim2','genh','codim1','POfold','print_residual_info',1);
%%
LPCbranch=br_contn(LPCfuncs,LPCbranch,20,'plotaxis',ax2);
%% plot 2d bif
figure(2);clf;
