...
if strcmp(p.kind,'psol')    
  if isempty(p.mesh)
    p.mesh=0:1/(size(p.profile,2)-1):1;
  end
  rho=method.minimal_modulus;
  max_number=method.max_number_of_eigenvalues;
  if ~funcs.tp_del
      if isfield(method,'sparse')
          sparse=method.sparse;
      else
          sparse=options.sparse;
      end
      mu=mult_app(funcs,p,rho,max_number,pass_on{:},'sparse',sparse);
  else 
    d_ac=method.delay_accuracy;
    mu=mult_app(funcs,p,rho,max_number,'d_ac',d_ac);
  end;
  if ~isempty(mu)
