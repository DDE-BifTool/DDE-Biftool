...
%% optional
default={'nf',size(pt.profile,1),'period',true,'output','Jacobian'};
...
% W{t_i}{1}) is W for delay t_i-1 (t_i=1 corresponds to tau=0), W{t_i}{2}
% is W' at delay t_i-1.
W=cell(1,d);
for t_i=1:d
...
f=f(:);
for t_i=d:-1:1
    dfdx{t_i}=funcs.sys_deri(xxvec,pt.parameter,t_i-1,[],[]);
