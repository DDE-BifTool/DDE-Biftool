...
for i=1:max_iter
    ...
    switch point.kind
        ...
        case 'psol'
            % produce jacobian
            [J,res,~,~,Jstruc]=psol_jac_complete(funcs,point,free_par,'c',col,...
                'step_cnd',step_cnd,'extra_cond',{resi,condi});
            % solve linear system
            if size(J,1)~=size(J,2)
                warning('p_correc:nonsquare','P_CORREC warning: use of nonsquare Jacobian.');
            end;
            dx=J\res;
            % apply non-parameter corrections
            point.profile(:)=point.profile(:)-dx(Jstruc.cols.xrg);
            point.period=point.period-dx(Jstruc.cols.T);
