...
\left\{
\begin{aligned}
\frac{\d}{\d t}{x(t)}&=f(x_0,x_1,\ldots,x_m,\eta)\mbox{,\quad with}\\
...
derivatives geenrated with the symbolic toolbox do not require the
...
\begin{quote}
  \textbf{Note}: See alternative \blist{sys_dirderi} in
  \cref{sec:dirderi}, providing directional derivatives.
\end{quote}
...
\end{itemize}
The set of all deriviatives up to \blist{maxorder} will be passed on
in a $1\times\blist{maxorder}$ cell array to \blist{set_funcs} (see
...
\Cref{neuron_sys_dirderi}.  Directional derivatives are also easier to
vectorize.
...
additional scalar variables $x_{n+_j}$ and add the $m$ equations
$0=x_{n+j}(t)-\tau_j(x(t),x(t-\tau_1),\ldots,x(t-\tau_m))$ as part of
the right-hand side. Then one may define the $j$th delay as equal to
$x_{n+j}$.
...
\item \blist{xx} ($n\times\blist{ind}$ matrix) is the state:
  \blist{xx(:,1)}$=x(t)$, \blist{xx(:,k)}$=x(t-\tau_{k-1})$ for
  $k=2\ldots\blist{ind}$;
\item \blist{par}$\in\RR^{1\times p}$ arethe parameters (as for the right-hand side)
\item \blist{nx} (empty, one integer or two integers) index (indices) of
  \blist{xx} with respect to which the right-hand side is to be
  differentiated;
\item \blist{np} (empty or integer) whether right-hand side is to be
...
differences.  Directional derivatives are also easier to vectorize.
